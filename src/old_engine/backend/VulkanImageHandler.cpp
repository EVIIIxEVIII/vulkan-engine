...
void Vulkan::ImageHandler::createImage(ImageProperties imageProperties,
                                     VkImage &image,
  ...
  allocInfo.memoryTypeIndex = m_bufferManager.findMemoryType(
      memRequirements.memoryTypeBits, imageProperties.memoryProperties);
...
bool Vulkan::ImageHandler::hasStencilComponent(VkFormat format) {
  return format == VK_FORMAT_D32_SFLOAT_S8_UINT ||
         format == VK_FORMAT_D24_UNORM_S8_UINT;
}

VkImageView Vulkan::ImageHandler::createImageView(VkImage image, VkFormat format,
...
void Vulkan::ImageHandler::transitionImageLayout(VkImage image, VkFormat format,
                                               VkImageLayout oldLayout,
                                               VkImageLayout newLayout,
                                               uint32_t mipLevels) {
  VkCommandBuffer commandBuffer =
      m_singleTimeCommand.beginSingleTimeCommands(m_context);
...
void Vulkan::ImageHandler::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {

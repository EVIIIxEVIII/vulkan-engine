...
#include "./engine/backend/ShaderResources/VulkanUBO.h"
#include "./engine/backend/ShaderResources/UBOStructs.h"
...
public:
    HelloTriangleApplication(
        ...
        UBO<TransformationUBO>& transformationUBO,
        ...
        singleTimeCommand(singleTimeCommand),
        imageHandler(imageHandler),
        transformationUBO(transformationUBO),
        descriptors(descriptors),
        graphicsPipeline(graphicsPipeline)
    {}
...
private:
    ...
    void cleanup() {
        ...
        transformationUBO.cleanup(MAX_FRAMES_IN_FLIGHT);
        descriptors.cleanup(context);
    ...
    void createDepthResources() {
        VkFormat depthFormat = graphicsPipeline.findDepthFormat(context);

        ImageProperties imageProperties{};
        imageProperties.width = swapChain.getExtent().width;
        imageProperties.height = swapChain.getExtent().height;
        imageProperties.mipLevels = 1;
        imageProperties.numSamples = context.getMsaaSamples();
        imageProperties.format = depthFormat;
        imageProperties.tiling = VK_IMAGE_TILING_OPTIMAL;
        imageProperties.usage = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
        imageProperties.memoryProperties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
        ...
        depthImageView = imageHandler.createImageView(depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 1);

        imageHandler.transitionImageLayout(depthImage, depthFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, 1);
    }
    ...
    void createColorResources() {
        ...
        imageProperties.mipLevels = 1;
        imageProperties.numSamples = context.getMsaaSamples();
        imageProperties.format = colorFormat;
        imageProperties.tiling = VK_IMAGE_TILING_OPTIMAL;
        imageProperties.usage = VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
        imageProperties.memoryProperties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
        ...
        colorImageView = imageHandler.createImageView(colorImage, colorFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);
    }
    ...
    void createTextureImage() {
        ...
        ImageProperties imageProperties{};
        imageProperties.width = texWidth;
        imageProperties.height = texHeight;
        imageProperties.mipLevels = mipLevels;
        imageProperties.numSamples = VK_SAMPLE_COUNT_1_BIT;
        imageProperties.format = VK_FORMAT_R8G8B8A8_SRGB;
        imageProperties.tiling = VK_IMAGE_TILING_OPTIMAL;
        imageProperties.usage = VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
        imageProperties.memoryProperties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT;
        ...
        imageHandler.transitionImageLayout(
            ...
            imageHandler.copyBufferToImage(stagingBuffer, textureImage, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
    ...
    void createTextureImageView() {
        textureImageView = imageHandler.createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
    }
    ...
    void createDescriptorSets() {
        std::vector<VkDescriptorSetLayout> layouts(MAX_FRAMES_IN_FLIGHT, descriptors.getDescriptorSetLayout());
        VkDescriptorSetAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
        allocInfo.descriptorPool = descriptors.getDescriptorPool();
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            descriptorWrites[0] = transformationUBO.createDescriptorWrite(descriptorSets[i], i);
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipeline.getGraphicsPipelineLayout(), 0, 1, &descriptorSets[currentFrame], 0, nullptr);
    ...
    void createSyncObjects() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            if (
                vkCreateSemaphore(context.getDevice(), &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS
                ||
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.proj[1][1] *= -1;
        transformationUBO.updateUBO(currentFrame, ubo);
    }
};
...
int old_main() {
    ...
    BufferManager bufferManger(context, singleTimeCommand);

    TransformationUBO transformationStructUBO{};
    transformationStructUBO.proj = glm::mat4();
    transformationStructUBO.view = glm::mat4();
    transformationStructUBO.model = glm::mat4();

    UBO<TransformationUBO> transformationUBO(context, bufferManger, transformationStructUBO, MAX_FRAMES_IN_FLIGHT);

    ImageHandler imageHandler(context, singleTimeCommand, bufferManger);

    Descriptors descriptors;
    descriptors.init(context, MAX_FRAMES_IN_FLIGHT);

    GraphicsPipeline graphicsPipeline;
    graphicsPipeline.init(context, window, swapChain, descriptors);
